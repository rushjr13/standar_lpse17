<div class="card shadow border-primary">
    ...
    <?php if($akses_menu>0){ ?>
  		<button type="button" class="btn shadow-sm btn-sm btn-circle btn-primary float-right" data-toggle="modal" data-target="#tambahModal" title="Tambah SK Koordinator Resiko Layanan"><i class="fa fa-fw fa-plus"></i></button>
    <?php } ?>
	</div>
	<div class="card-body table-responsive">
		<table class="table table-sm table-hover shadow-sm table-bordered table-striped" id="dataTable" width="100%" cellspacing="0">
      ...
      <tbody>
        <?php if($resiko_sk){ ?>
          <?php $no=1; foreach ($resiko_sk as $sk): ?>
          <?php
          ...
          ?>
            <tr>
              ...
              <td class="align-middle text-center"><button type="button" class="btn btn-sm btn-info shadow-sm" id="file" data-toggle="modal" data-target="#fileModal" title="Lihat File <?=$sk['file'] ?>" data-nomor="<?=$sk['nomor'] ?>" data-tanggal="<?=$tanggal_sk ?>" data-nama="<?=$sk['nama'] ?>" data-file="<?=$sk['file'] ?>"><i class="fa fa-fw fa-file-pdf"></i><?=$sk['nama'] ?></button></td>
              ...
              <?php if($akses_menu>0){ ?>
                <td class="align-middle text-center">
                  <button tipe="button" class="btn shadow-sm btn-sm btn-circle btn-info" id="ubah" data-toggle="modal" data-target="#ubahModal" title="Ubah" data-id="<?=$sk['id'] ?>" data-nomor="<?=$sk['nomor'] ?>" data-tanggal="<?=$sk['tanggal'] ?>" data-nama="<?=$sk['nama'] ?>" data-tentang="<?=$sk['tentang'] ?>" data-berlaku="<?=$sk['berlaku'] ?>" data-berakhir="<?=$sk['berakhir'] ?>" data-file="<?=$sk['file'] ?>"><i class="fa fa-fw fa-edit"></i></button>
                  <button tipe="button" class="btn shadow-sm btn-sm btn-circle btn-danger" id="hapus" data-toggle="modal" data-target="#hapusModal" title="Hapus" data-id="<?=$sk['id'] ?>" data-nomor="<?=$sk['nomor'] ?>" data-tanggal="<?=$tanggal_sk ?>" data-nama="<?=$sk['nama'] ?>" data-file="<?=$sk['file'] ?>"><i class="fa fa-fw fa-trash"></i></button>
                </td>
              <?php } ?>
            </tr>
          <?php endforeach ?>
  ...
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="tambahModalLabel">Tambah SK Koordinator Resiko Layanan</h5>
      </div>
      <form id="formtambah" action="<?=base_url('resiko/sk/tambah') ?>" method="post" enctype="multipart/form-data">
...
<div class="modal fade" id="ubahModal" tabindex="-1" role="dialog" aria-labelledby="ubahModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="ubahModalLabel">Tambah SK Koordinator Resiko Layanan</h5>
      </div>
      <form id="formubah" action="<?=base_url('resiko/sk/ubah') ?>" method="post" enctype="multipart/form-data">
        <div class="modal-body">
          <div class="form-group row">
            <label for="id" class="col-sm-2 col-form-label">Kode SK</label>
            <div class="col-sm-10">
...
<div class="modal fade" id="hapusModal" tabindex="-1" role="dialog" aria-labelledby="hapusModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="hapusModalLabel">Hapus SK Koordinator Resiko Layanan</h5>
      </div>
...
<div class="modal fade" id="fileModal" tabindex="-1" role="dialog" aria-labelledby="fileModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="fileModalLabel">File SK Koordinator Resiko Layanan</h5>
...
<script>
    ...
    $(document).on("click", "#hapus", function() {
        ...
        $("#hapusModal #formhapus").attr('action', '<?=base_url('resiko/sk/hapus/') ?>'+id);
    });
    ...
    $(document).on("click", "#ubah", function() {
        ...
        $("#ubahModal #formubah").attr('action', '<?=base_url('resiko/sk/ubah/') ?>'+id);
    });
