<div class="row justify-content-md-center">
  <div class="col-8">
    <form action="<?=base_url('resiko/form/sdm/ubah/'.$resiko_sdm['id']) ?>" method="post">
      <div class="card shadow border-primary">
        ...
        <div class="card-body">
          <div class="form-group row">
            ...
            <div class="col-sm-10">
              <input type="text" readonly class="form-control-plaintext font-weight-bold" id="id" name="id" value="<?=$resiko_sdm['id'] ?>">
            </div>
          </div>
          <div class="form-group row">
            ...
            <div class="col-sm-10">
              <select class="custom-select shadow-sm" id="klasifikasi" name="klasifikasi">
                ...
                <?php foreach ($klasifikasi_sdm as $ksdm): ?>
                  <option value="<?=$ksdm['id_ksdm'] ?>" <?php if($resiko_sdm['id_ksdm']==$ksdm['id_ksdm']){echo "selected";} ?>><?=$ksdm['kla_sdm'] ?></option>
                <?php endforeach ?>
              </select>
            ...
            <div class="col-sm-10">
              <select class="custom-select shadow-sm" id="dampak" name="dampak">
                ...
                <?php foreach ($resiko_dampak as $rd): ?>
                  <option value="<?=$rd['nilai'] ?>" <?php if($resiko_sdm['dampak']==$rd['nilai']){echo "selected";} ?>><?=$rd['nilai'] ?>. <?=$rd['ekonomi'] ?>, <?=$rd['reputasi'] ?>, <?=$rd['pidana'] ?>, <?=$rd['kinerja'] ?></option>
                <?php endforeach ?>
              </select>
            ...
            <div class="col-sm-10">
              <select class="custom-select shadow-sm" id="pengancam" name="pengancam">
                ...
                <?php foreach ($resiko_pengancam as $rp): ?>
                  <option value="<?=$rp['nilai'] ?>" <?php if($resiko_sdm['pengancam']==$rp['nilai']){echo "selected";} ?>><?=$rp['nilai'] ?>. <?=$rp['profil_pengancam'] ?></option>
                <?php endforeach ?>
              </select>
            ...
            <div class="col-sm-10">
              <select class="custom-select shadow-sm" id="kerentanan" name="kerentanan">
                ...
                <?php foreach ($resiko_rentan as $rr): ?>
                  <option value="<?=$rr['nilai'] ?>" <?php if($resiko_sdm['kerentanan']==$rr['nilai']){echo "selected";} ?>><?=$rr['nilai'] ?>. <?=$rr['tingkat_rentan'] ?></option>
                <?php endforeach ?>
              </select>
            ...
            <div class="col-sm-10">
              <select class="custom-select shadow-sm" id="paparan" name="paparan">
                ...
                <?php foreach ($resiko_paparan as $rpap): ?>
                  <option value="<?=$rpap['nilai'] ?>" <?php if($resiko_sdm['paparan']==$rpap['nilai']){echo "selected";} ?>><?=$rpap['nilai'] ?>. <?=$rpap['contoh_paparan'] ?></option>
                <?php endforeach ?>
              </select>
